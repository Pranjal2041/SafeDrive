...
import com.pranjal.model_renderer.RenderingColorClass;
...
public class ModelRenderer implements GLSurfaceView.Renderer {
    ...
    private void drawObject(float[] viewMatrix, float[] projectionMatrix, float[] lightPosInWorldSpace, float[] colorMask, float[] cameraPosInWorldSpace, boolean doAnimation, boolean drawLighting, boolean drawWireframe, boolean drawTextures, boolean drawColors, List<Object3DData> objects, int i) {
        ...
        try {
            ...
            if(scene.getColorMapping()!=null) {
                if (objData.getElements() != null) {
                    int asd = -1;
                    for (Element element : objData.getElements()) {
                        asd++;
                        RenderingColorClass color = scene.getColorMapping().get(asd);
                        if (element.getMaterial() == null) {
                            Log.d(TAG, "null materail......");
                            continue;
                        }
                        if(color == null) {
//                            element.getMaterial().setAmbient(new float[]{1.0f,1.0f,1.0f,1.0f});
//                            element.getMaterial().setDiffuse(new float[]{1.0f,1.0f,1.0f,1.0f});
//                            element.getMaterial().getColor();
//                            android.util.Log.d(TAG, "drawObject: color null");
                        }
                        else {
//                            element.getMaterial().setAmbient(new float[]{1.0f,1.0f,1.0f,1.0f});
//                            element.getMaterial().setDiffuse(new float[]{1.0f,1.0f,1.0f,1.0f});
//                            element.getMaterial().setColor(new float[]{1.0f,1.0f,1.0f,1.0f});
                            element.getMaterial().setAmbient(color.getAmbientColor());
                            element.getMaterial().setDiffuse(color.getDiffuseColor());
                            element.getMaterial().setColor(color.getDiffuseColor());
//                            element.getMaterial().getColor();
                            android.util.Log.d(TAG, "drawObject: color ="+color.getAmbientColor());
                        }
                    }
                    Log.d(TAG, "drawObject: drawn " + asd + " elements");
